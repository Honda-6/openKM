				...
				for (DiskFileItem item : items) {
					if (item.isFormField()) {
						if (item.getFieldName().equals("action")) {
							...
							ct.setId(Integer.parseInt(item.getString(StandardCharsets.UTF_8)));
						} else if (item.getFieldName().equals("ct_name")) {
							ct.setName(item.getString(StandardCharsets.UTF_8));
						} else if (item.getFieldName().equals("ct_mail")) {
							ct.setMail(item.getString(StandardCharsets.UTF_8));
						} else if (item.getFieldName().equals("ct_expression")) {
							ct.setExpression(item.getString(StandardCharsets.UTF_8));
